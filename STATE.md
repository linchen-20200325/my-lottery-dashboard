...
- [x] **T2-a `ui/_view_base.py`(新)+ `tests/test_view_base.py`(新)**:抽兩 view 純 helper SSOT — `expand_tails_to_numbers`(S4)/`freshness_warning`/`upload_provenance`/`analysis_rng`(DR-7 RNG 契約);無 streamlit 依賴 → 8 單元測試覆蓋。待 T2-b/c 把兩 view 接上 + 修 DR-2/4/5。
